...
int			create_texture_cube(const char *folder)
{
	...
	while (++i < 6)
	{
		...
		GLCall(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA8, local_buffer->w, local_buffer->h, 0, GL_RGBA, GL_UNSIGNED_BYTE, local_buffer->pixels));
...
int			create_texture_2d(const char *filename)
{
	SDL_Surface	*local_buffer;
	GLuint		texture_id;
